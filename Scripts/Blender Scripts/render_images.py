...
def colorChoice(color): #manually defined colors
    if color == 0:
...
#helps export data, folder where user chooses to export images!!!
...
#creates some UI
#creates a new tab on the 3D viewport (right side), called "UAV Render?"
#tab has a button that will render and export images, user has to click "Render and Export" and then "Export Data"
class RenderPanel(bpy.types.Panel):
    bl_label = "Render Training Images"
    bl_idname = "PT_RenderPanel"
    bl_space_type = 'VIEW_3D'
    bl_region_type = 'UI'
    bl_category = 'UAV Render?'
    
    def draw(self, context):
        scene = context.scene
        layout = self.layout
        row = layout.row()
        row.label(text = "Ready to render?", icon = 'RENDER_STILL')
        row = layout.row()
        row.operator("export_test.data", text = "Render and Export")  
